  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('excel_files').select('*').eq('id', fileIdFromUrl).single();
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        ...
        `).eq('session_id', selectedSessionId).maybeSingle();
      if (error) throw error;
      return data;
    },
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('chat_messages').select('*, excel_files(filename, file_size)').eq('session_id', selectedSessionId).order('created_at', {
        ascending: true
      });
      if (error) throw error;
      return data;
  ...
  const handleSendMessage = async (message: string, fileId?: string | null) => {
    ...
    try {
      ...
      const {
        data: analysis,
        error
      } = await supabase.functions.invoke('excel-assistant', {
        body: {
  ...
  return <SidebarProvider>
      <div className="flex min-h-screen w-full bg-gray-50">
        ...
        <div className="flex-1 flex flex-col transition-all duration-200 ml-[60px] sidebar-expanded:ml-[300px]">
          {!selectedSessionId ? <div className="flex-grow flex items-center justify-center px-4 lg:px-6">
              ...
              opacity: 0
                  ...
                  <ScrollArea className="flex-grow p-4">
                    <div className="space-y-6">
                      {messages.map(msg => <MessageContent key={msg.id} content={msg.content} role={msg.role as 'user' | 'assistant'} timestamp={formatTimestamp(msg.created_at)} fileInfo={msg.excel_files} />)}
                    </div>
                  </ScrollArea>
                </motion.div>
              </div>
