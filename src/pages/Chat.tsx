...
const Chat = () => {
  ...
  const { data: messages, isLoading: messagesLoading, refetch: refetchMessages } = useQuery({
    queryKey: ['chat-messages', selectedThreadId || fileId],
    queryFn: async () => {
      if (!selectedThreadId && !fileId) return [];
      const { data, error } = await supabase
        .from('chat_messages')
        .select('*')
        .eq(selectedThreadId ? 'session_id' : 'excel_file_id', selectedThreadId || fileId)
        .order('created_at', { ascending: true });
      
      if (error) throw error;
      return data;
    },
    enabled: !!(selectedThreadId || fileId),
  });
